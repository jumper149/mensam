...
Any version number `BREAK.MAJOR.MINOR.PATCH` already tells the operator of a deployment some basic information.

`BREAK`:: Manual intervention required. Read the link:./CHANGELOG.adoc[changelog]!
`MAJOR`:: Major changes. These changes break backwards compatibility, often including automated database migrations.
