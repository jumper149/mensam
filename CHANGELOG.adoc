...
* Frontend
  ** Fix calculation, when reservations clash, for preview.

== `1.0.1.0`

* Frontend
  ** Add time range previews to room view.
...
* Deployment
  ** Fix NixOS module.
    *** Add the optional option `services.mensam.dockerImage`.

== `1.0.0.0`

* Deployment
  ** Default location of configuration was moved from `/etc/mensam/mensam.json` to `/etc/mensam/configuration.json` for the Docker image.
  ** NixOS is now also using `/etc/mensam/configuration.json` to link the generated configuration.
* Frontend
  ** Add some pictograms.
...
* Deployment
  ** NixOS module now supports using the prebuilt Docker image.
* Frontend
...
* Frontend
  ** Desks can be viewed in a top-down "Room" view.
